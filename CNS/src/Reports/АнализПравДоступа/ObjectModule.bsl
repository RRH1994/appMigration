...
Функция ВариантСОграничениямиДоступа()
	...
	Если ТипЗнч(КомпоновщикНастроек.Настройки.ДополнительныеСвойства)=Тип("Структура")
			И КомпоновщикНастроек.Настройки.ДополнительныеСвойства.Свойство("КлючПредопределенногоВарианта", КлючПредопределенногоВарианта)
			И Не КлючПредопределенногоВарианта = Неопределено Тогда 
	
		Возврат "ПраваПользователяНаТаблицу" = КлючПредопределенногоВарианта; // КомпоновщикНастроек.Настройки.ДополнительныеСвойства.КлючПредопределенногоВарианта;
		
	Иначе
		Возврат Истина; //Всегда предполагаем, что включен RLS
	КонецЕсли;	
	//
